...
class PythonProcessGroupExtensionTest(MultiProcessTestCase):
    ...
    def test_backend_config(self):
        ...
        if TEST_XPU:
            backend_config_strings_and_expected_values.remove((dist.Backend.DUMMY, "cpu:dummy,cuda:dummy"))
            backend_config_strings_and_expected_values.remove(("DUMMY", "cpu:dummy,cuda:dummy"))
            backend_config_strings_and_expected_values.remove(("dummy", "cpu:dummy,cuda:dummy"))
        ...
        for config_str, expected_value in backend_config_strings_and_expected_values:
            print("####### {} {}".format(config_str, expected_value), flush=True)
...
if __name__ == "__main__":
    if TEST_CUDA:
        assert not torch.cuda._initialized, (
            f"test_distributed must not have initialized cuda context on main process"
        )
