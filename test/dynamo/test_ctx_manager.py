...
requires_gpu = unittest.skipIf(not TEST_CUDA and not TEST_XPU, "requires cuda or xpu")
...
class CtxManagerTests(torch._dynamo.test_case.TestCase):
    ...
    def test_gpu_stream_context_manager2(self):
        def fn(x, s):
            ...
            current_stream.wait_stream(s)
            with self.mod.stream(current_stream):
                x = torch.relu(x)

            s2 = self.mod.Stream()
            s2.wait_stream(current_stream)
            with self.mod.stream(s2):
                x = torch.relu(x)
    ...
    def test_gpu_event_created_outside_of_graph(self):
        ...
        def run_iters(fn, compile=False):
            ...
            for _ in range(10):
                with self.mod.stream(user_stream):
    ...
    def test_gpu_device(self):
        def fn(x):
            with self.mod.device(x.device.index - 1):
                x = torch.sin(x + 1)
            return x
    ...
    def test_autocast(self):
        if not self.mod.is_bf16_supported():
    ...
    def test_autocast_sdpa(self):
        ...
        head_dim = 8
        query = torch.ones(
            1, 8, seq_len_q, head_dim, device=GPU_TYPE, dtype=dtype, requires_grad=True
        )
        key = torch.ones(
            1, 8, seq_len_k, head_dim, device=GPU_TYPE, dtype=dtype, requires_grad=True
        )
        value = torch.ones(
            1, 8, seq_len_k, head_dim, device=GPU_TYPE, dtype=dtype, requires_grad=True
        )
        ...
        self.assertEqual(compiled.device.type, GPU_TYPE)
    ...
    def test_graph_break_inlining_autocast(self):
        for device in [GPU_TYPE, "cpu"]:
            if device == GPU_TYPE and not (
                torch.accelerator.is_available() and self.mod.is_bf16_supported()
            ):
                continue
