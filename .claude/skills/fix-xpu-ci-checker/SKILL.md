...
5. **Do not** require local pytest execution — diagnosis here is log-driven. Test execution is a separate skill (`submit_ut_issues`, `port-pytorch-tests-xpu`).
...
- `port-pytorch-tests-xpu` / `port-cuda-tests-xpu` — when the PR-owned fix is actually a porting-convention gap.
